...
<?php $this->renderPartial('_services', array('data' => $services)) ?>
...
<script>
    ...
    function a_imgUpload(type,i){
        ...
        $('#'+type+'-'+i).fileupload({
            ...
            dataType: 'json',
                ...
                if($('.img'+type+'-'+i+' a img').length > 0){
                    $('.img'+type+'-'+i+' a img').remove();
                }
                ...
                $.each(data.result.files, function (index, file) {
                    ...
                    $('.h'+type+'-'+i+' a').remove();
                    $('.h'+type+'-'+i).append(
                            '<a href="#." onclick="triggerClickF('+"'"+type+"'"+','+i+')" class="thumb-edit">Edit</a><a href="#." onclick="delete_img('+"'"+type+"'"+','+i+','+"'"+file.dirpath+"'"+','+"'"+file.thumbnail_dir+"'"+')" class="thumb-delete">Delete</a>'
                    );
                    ...
                    $('.img'+type+'-'+i+' a').append(
                            $('<input />').attr('type','hidden').attr('name', 'puraob').attr('value',data));

                    $('.img'+type+'-'+i+' a').append(
                            $('<input />').attr('type','hidden').attr('name', type+'[images]['+i+'][thumb_path]').attr('value',file.thumbnail_dir).after(
                                    $('<input />').attr('type','hidden').attr('name',type+'[images]['+i+'][path]').attr('value',file.dirpath).after(
                                            $('<input />').attr('type','hidden').attr('name',type+'[images]['+i+'][name]').attr('value',file.name).after(
                                                    $('<input />').attr('type','hidden').attr('name',type+'[images]['+i+'][size]').attr('value',file.size).after(
                                                            $('<input />').attr('type','hidden').attr('name',type+'[images]['+i+'][type]').attr('value',file.type)
                                                    )
                                            )
                                    )
                            )
    ...
    function delete_img(type, i, filepath, thumbpath){
        //$(i).remove();
        $('.img'+type+'-'+i+' a img').remove();
        $('.h'+type+'-'+i+' a').remove();
        $('.h'+type+'-'+i).append(
                '<a href="#." onclick="triggerClickF('+"'"+type+"'"+','+i+')">upload</a>'
        );

        $.ajax({
            url: "/property/tempuploaddel",
            type: "POST",
            data: "filepath="+filepath+"&thumbpath="+thumbpath,
