...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            ...
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/resources/files/',
            'upload_url' => $this->get_full_url().'/resources/files/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function handle_image_file($file_path, $file) {
        ...
        foreach($this->options['image_versions'] as $version => $options) {
            if ($this->create_scaled_image($file->name, $version, $options)) {
                if (!empty($version)) {
                    $file->{$version.'_dir'} = urlencode($this->get_upload_path($file->name, $version));
                    $file->{$version.'_url'} = $this->get_download_url(
                        $file->name,
                        $version
                    );
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file = new stdClass();
        $file->dirpath = urlencode($this->get_upload_path($name));
